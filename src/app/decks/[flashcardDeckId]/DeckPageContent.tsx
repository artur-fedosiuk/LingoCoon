      ...
      <div className="container mx-auto p-6 space-y-8 max-w-6xl">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b pb-6">
          <div className="flex-1">
            ...
            >
              ...
              {t('decks.back_to_decks', 'Back to Decks')}
            </Link>
            ...
            {cards && cards.length > 0 && (
              <Link href={`/decks/${deck.id}/study`}>
                <button className="bg-black text-white px-6 py-3 rounded-lg font-medium hover:bg-gray-800 flex items-center gap-2 shadow-lg">
                  ...
                  {t('decks.start_study', 'Start Study Session')}
                </button>
              </Link>
            )}
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div className="bg-gray-50 p-6 rounded-xl border border-gray-200 h-fit">
            <h2 className="font-semibold text-lg mb-4 text-gray-900">{t('flashcards.create_new', 'Add New Card')}</h2>
            <CreateCardForm
              deckId={deck.id}
          ...
          <div className="space-y-4">
            <h2 className="font-semibold text-lg flex justify-between items-center text-gray-900">
              {t('decks.cards_in_deck', 'Cards in Deck')}
            ...
            {!cards || cards.length === 0 ? (
              <div className="text-center py-16 text-gray-400 border-2 border-dashed border-gray-300 rounded-xl bg-gray-50">
                <p className="text-lg mb-2">{t('flashcards.empty_state', 'No cards yet')}</p>
                <p className="text-sm">{t('flashcards.empty_state_cta', 'Add one on the left!')}</p>
              </div>
            ) : (
